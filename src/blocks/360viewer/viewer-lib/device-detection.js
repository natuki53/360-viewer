...
export function detectWebGLSupport() {
    try {
        ...
        if (globalDeviceInfo && globalDeviceInfo.isMobile) {
            if (DEBUG_MODE) {
                console.log('Mobile device: WebGL context available, skipping advanced tests');
            }
            return true;
        }
...
export function detectDevicePerformanceLevel() {
    try {
        ...
        if (debugInfo) {
            ...
            // ソフトウェアレンダリングの場合は低性能
            if (rendererLower.includes('software') || rendererLower.includes('llvmpipe')) {
                return 'low';
            }
            ...
            if (vendorLower.includes('nvidia')) {
                // NVIDIA GPU
                if (rendererLower.includes('rtx') || rendererLower.includes('gtx 10') || 
                    rendererLower.includes('gtx 16') || rendererLower.includes('gtx 20') ||
                    rendererLower.includes('gtx 30') || rendererLower.includes('gtx 40')) {
                    return 'high';
                } else if (rendererLower.includes('gtx') || rendererLower.includes('gt')) {
                    return 'medium';
                }
            } else if (vendorLower.includes('amd') || vendorLower.includes('ati')) {
                // AMD GPU - 詳細な判定
                if (rendererLower.includes('rx 6') || rendererLower.includes('rx 7') ||
                    rendererLower.includes('rx 8') || rendererLower.includes('radeon rx')) {
                    return 'high';
                } else if (rendererLower.includes('rx 5') || rendererLower.includes('rx 4')) {
                    return 'medium';
                } else if (rendererLower.includes('rx') || rendererLower.includes('radeon')) {
                    return 'medium';
                } else if (rendererLower.includes('vega') || rendererLower.includes('raven ridge') ||
                          rendererLower.includes('picasso') || rendererLower.includes('renoir')) {
                    return 'medium';
                } else if (rendererLower.includes('radeon r') || rendererLower.includes('radeon hd')) {
                    return 'low';
                }
                ...
                if (rendererLower.includes('arc')) {
                    return 'high';
                } else if (rendererLower.includes('iris xe') || rendererLower.includes('iris plus')) {
                    return 'medium';
                } else if (rendererLower.includes('uhd') || rendererLower.includes('hd graphics')) {
                    const hdMatch = rendererLower.match(/hd graphics (\d+)/);
                    const uhdMatch = rendererLower.match(/uhd graphics (\d+)/);
                    ...
                    if (hdMatch) {
                        ...
                        return 'low';
                    } else if (rendererLower.includes('uhd 7') || rendererLower.includes('uhd 6')) {
                        return 'low';
                    } else {
                        return 'low';
                    }
                ...
                // Apple Silicon
                if (rendererLower.includes('apple m1') || rendererLower.includes('apple m2') ||
                    rendererLower.includes('apple m3') || rendererLower.includes('apple m4')) {
                    return 'high';
                } else {
                    return 'medium';
                }
