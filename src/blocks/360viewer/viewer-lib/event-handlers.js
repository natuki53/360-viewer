...
export class EventHandlers {
	...
	setupEventListeners() {
		...
		container.addEventListener(
			'touchstart',
			this.boundOnDocumentTouchStart,
			false
		);
		container.addEventListener(
			'touchmove',
			this.boundOnDocumentTouchMove,
			false
		);
		container.addEventListener(
			'touchend',
			this.boundOnDocumentTouchEnd,
			false
		);
		container.addEventListener(
			'click',
			this.boundOnContainerClick,
			false
		);
	...
	onDocumentMouseMove( event ) {
		if ( this.viewer.isUserInteracting ) {
			this.viewer.lon =
				( this.viewer.onMouseDownMouseX - event.clientX ) * 0.1 +
				this.viewer.onMouseDownLon;
			this.viewer.lat =
				( this.viewer.onMouseDownMouseY - event.clientY ) * 0.1 +
				this.viewer.onMouseDownLat;
		}
		this.updateCursor();
	}
	...
	isButtonOrErrorClick( target ) {
		return (
			target === this.viewer.zoomInButton ||
			target === this.viewer.zoomOutButton ||
			( ! this.viewer.isMobile &&
				target === this.viewer.fullscreenButton ) ||
