...
export class UIControls {
	...
	createZoomButtons() {
		...
		this.viewer.zoomInButton.className =
			'psv-btn psv-zoom-btn psv-zoom-in-btn';
		...
		this.viewer.zoomInButton.addEventListener(
			'touchend',
			this.boundZoomInTouch,
			{ passive: false }
		);
		...
		this.viewer.zoomOutButton.className =
			'psv-btn psv-zoom-btn psv-zoom-out-btn';
		...
		this.viewer.zoomOutButton.addEventListener(
			'touchend',
			this.boundZoomOutTouch,
			{ passive: false }
		);
	...
	removeEventListeners() {
		if ( this.viewer.zoomInButton ) {
			...
			if ( this.boundZoomInTouch ) {
				this.viewer.zoomInButton.removeEventListener(
					'touchend',
					this.boundZoomInTouch
				);
			}
		...
		if ( this.viewer.zoomOutButton ) {
			...
			if ( this.boundZoomOutTouch ) {
				this.viewer.zoomOutButton.removeEventListener(
					'touchend',
					this.boundZoomOutTouch
				);
			}
	...
	dispose() {
		...
		if ( this.viewer.zoomInButton && this.viewer.zoomInButton.parentNode ) {
			this.viewer.zoomInButton.parentNode.removeChild(
				this.viewer.zoomInButton
			);
		}
		if (
			this.viewer.zoomOutButton &&
			this.viewer.zoomOutButton.parentNode
		) {
			this.viewer.zoomOutButton.parentNode.removeChild(
				this.viewer.zoomOutButton
			);
		}
		if (
			this.viewer.fullscreenButton &&
			this.viewer.fullscreenButton.parentNode
		) {
			this.viewer.fullscreenButton.parentNode.removeChild(
				this.viewer.fullscreenButton
			);
		}
