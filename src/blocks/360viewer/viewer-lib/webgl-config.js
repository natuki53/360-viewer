...
export function getGlobalWebGLConfig() {
    ...
    if (deviceInfo.isMobile) {
        if (deviceInfo.performanceLevel === 'high') {
            globalWebGLConfig.rendererOptions.powerPreference = 'high-performance';
            ...
            globalWebGLConfig.pixelRatio = Math.min(window.devicePixelRatio, 2.5);
            if (DEBUG_MODE) {
                console.log('Global WebGL config: High-performance mobile settings');
            }
        } else {
            ...
            globalWebGLConfig.rendererOptions.preserveDrawingBuffer = false;
            globalWebGLConfig.rendererOptions.failIfMajorPerformanceCaveat = false;
            globalWebGLConfig.pixelRatio = Math.min(window.devicePixelRatio, 2.0);
            if (DEBUG_MODE) {
        ...
        if (deviceInfo.performanceLevel === 'high') {
            ...
            globalWebGLConfig.rendererOptions.antialias = true;
            if (deviceInfo.isMac) {
                globalWebGLConfig.pixelRatio = Math.min(window.devicePixelRatio, 3);
            } else {
                globalWebGLConfig.pixelRatio = Math.min(window.devicePixelRatio, 2.5);
            }
            ...
            globalWebGLConfig.rendererOptions.powerPreference = 'low-power';
            globalWebGLConfig.rendererOptions.antialias = false;
            globalWebGLConfig.pixelRatio = Math.min(window.devicePixelRatio, 1.5);
        } else {
            globalWebGLConfig.rendererOptions.powerPreference = 'low-power';
            globalWebGLConfig.rendererOptions.antialias = false;
